...
/* A L I N A ' S   N O T E */
//
// I though a lot about how to use while to find the longest word, but couldn't do it
// Then I realized that if I sort ascendingly the array, then I can get the last element
// which would also be the biggest.
// I found the numberical sort function and used that.
// For the next exercise, I'm asked to add more lengths so the initial average of 6
// is not true anymore. I left it there, but the new average is now so the bool will return false
...
function findLongestWord( array_words ){
	...
	for (var index = 0; index < array_words.length; index++){
		lengths.push(array_words[index].length);	
	};
	function sortNumber(a,b) {
    return a - b;
	}
	var longestWord = lengths.sort(sortNumber)[lengths.sort(sortNumber).length-1];
	return longestWord;
};
...
array_lengths.push( findLongestWord( ["Alina", "Marjon", "Ben", "Ana", "Guillaume", "Bernadette"] ) );
array_lengths.push( findLongestWord( ["Me", "You", "Horse", "Cucumber", "Word", "Table"] ) );
...
function charFreq( string ) {
	...
	for (var i=0; i<string.length; i++){
		...
		if(!freqCounter[stringletter]){
			freqCounter[stringletter] = 1		
		}
		else {
			freqCounter[stringletter] = freqCounter[stringletter] + 1
		}
	};
